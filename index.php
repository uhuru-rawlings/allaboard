    ...
    <div class="body">
        ...
        <div class="content">
            <div class="map" id="map">
                <script
                src="https://maps.googleapis.com/maps/api/js?key=AIzaSyBKE0Xce6TlJ0VJ1icE1LYYYjU4HlH6bg4&callback=initMap&libraries=&v=weekly"
                async>
                </script>
            </div>
            <div class="recent_travels">
                Recent Travel
                <!-- php -->
                <?php
             include("includes/connection.php");
             $history = $_COOKIE['logedin'];
             $sql = "SELECT * FROM bookings WHERE user_email='".$history."' ORDER BY id DESC LIMIT 1";
             $query = mysqli_query($connection, $sql);
             if($rows = mysqli_num_rows($query) > 0){
